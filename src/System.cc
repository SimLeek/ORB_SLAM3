...
void System::SaveTrajectoryEuRoC(const std::string &filename)
{
    ...
    if (numMaxKFs == 0) {
        std::cout << "No maps have any keyframes to write. Not writing file." << std::endl;
        return; //pBiggerMap was never initialized in this case, and there's nothing to map anyway, so just close
    }
